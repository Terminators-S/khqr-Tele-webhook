...
- Dashboard payment-account architecture repaired: store payloads now expose every `PaymentSource` as `sources[]` while retaining the legacy first `source`. The selected payment QR account now drives Setup, Telegram group binding, Integration manifest, Real Payment Test and Activity instead of silently collapsing a store to its oldest source.
- Setup Step 2 now includes a **Payment QR account** selector and **Add / bulk upload QR** flow. Operators can upload 1–20 real static bank/Bakong KHQR images at once; each valid image becomes an independent payment account, exact original bytes are retained, duplicates are rejected, invalid files do not consume the store's blank/default source, and unexpected failures clean temporary source/assets rather than leaving ghost accounts.
- Real Payment Test operator controls rebuilt: `Check now` shows active Telegram scanning, MISMATCH remains actionable, terminal results expose **Start new test**, and `MISMATCH` can now actually be cancelled. **Paid but not verified?** adds exact ABA Trx-ID recovery over the configured trusted Telegram group; recovered evidence remains SHADOW and never allocates, emits a webhook or fulfills anything. An expired mismatch stays expired unless the exact recovered transaction safely verifies.
- Selected-source Integration now supports `?source_id=` and rejects sources outside the selected store. Store edits no longer rename/corrupt configured payment-account identities when multiple KHQR accounts exist.
- Multi-QR/recovery focused gate: 20/20 PASS. Full isolated regression after the operator rebuild: 84/84 PASS. JS syntax, Python compileall and git diff checks PASS.
