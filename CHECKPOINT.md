...
- Normal dashboard flow is now Home → Setup → Test payment → Activity → Advanced, with Setup = Store → Upload your KHQR → Telegram → Test. New Store and store switching are exposed directly in the UI.
- Telegram API credential entry remains in the UI; group selection saves the group for the selected store and automatically discovers/binds the unanimous ABA sender.
- Focused upload/store/acceptance/Telegram gate: 20/20 PASS. Full isolated PostgreSQL regression after upload-first + multi-store rebuild: 74/74 PASS. JS syntax, Python compileall and git diff check PASS.
...
- Upload-first dashboard deployed to Exosme and externally smoke-tested at the paired dashboard route. Post-deploy production state remains safe: 2 sources, 0 enabled, 0 allocations, 2 intents, 3 SHADOW evidence; TELEGRAM_SHADOW_ONLY=true, ALLOW_LIVE_TELEGRAM=false, ALLOW_SHADOW_PROMOTION=false.

## Next task
Have the operator hard-refresh the Exosme dashboard, select or create the intended store, and upload that store's real reusable static KHQR image. Confirm the dashboard decodes it and shows the exact uploaded image. Then finish/select the store's Telegram payment group and start a new small Real Payment Test. Confirm the ABA/Telegram notification reaches VERIFIED with exact amount/Remark while the source remains disabled and evidence stays SHADOW. After that, continue collecting SHADOW parity toward the separate 10-payment cutover gate.
