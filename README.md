...
7. From the consuming application's backend, create `/v1/payment-intents` with `X-Api-Key`, `Idempotency-Key`, the source ID, the application's own order/product `external_id`, integer minor-unit amount and optional metadata. Clients that need a short human-entry note may also send `remark_prefix` (for example BikeBoss uses `BB`). Product/catalog authority stays in the consuming application.
